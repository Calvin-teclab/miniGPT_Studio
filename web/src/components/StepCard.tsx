  ...
  return (
    ...
    >
      <div className="p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-text flex items-center gap-2">
                ...
                {detectedFromCache && (
                  <span className="px-2 py-0.5 rounded-full bg-success/10 text-success text-[11px] font-normal">
                    已检测到本地缓存
                  </span>
                )}
              </h3>
              ...
              {detectedFromCache && (
                <p className="text-xs text-success/90 mt-1">
                  这一步没有刚刚自动执行，而是后端在本机缓存目录中找到了已有产物。
                </p>
              )}
            </div>
          </div>
        ...
        {detail && expanded && (
          <div className="mt-4 border border-border rounded-xl bg-surface/60 overflow-hidden">
            ...
            <div className="p-4">
              <div className="text-sm font-medium mb-3">具体处理环节</div>
            ...
            <div className="px-4 py-3 bg-success/5 border-t border-success/20 text-xs text-success">
        ...
        {(status === 'pending' || status === 'error') && (
          <div className="flex gap-2 mt-4">
            ...
            {onSkip && (
              <button
                onClick={onSkip}
                className="px-4 py-2 rounded-lg text-sm text-text-muted hover:text-text hover:bg-surface-lighter transition-all"
              >
                跳过
              </button>
            )}
        ...
        {status === 'completed' && detectedFromCache && (
          <div className="flex gap-2 mt-4">
            <button
              onClick={onStart}
              className="px-4 py-2 rounded-lg text-sm font-medium bg-primary text-white hover:bg-primary-dark active:scale-[0.98] transition-all"
            >
              {rerunLabel}
            </button>
            {onUseCache && (
