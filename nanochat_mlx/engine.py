...
class Engine:
    ...
    def _mask_invalid_token_logits(self, logits):
        """Prevent sampling padded model-vocab ids that the tokenizer cannot decode."""
        valid_vocab_size = min(
            int(self.tokenizer.get_vocab_size()),
            int(getattr(self.model.config, "vocab_size", logits.shape[-1])),
            logits.shape[-1],
        )
        if valid_vocab_size >= logits.shape[-1]:
            return logits
        invalid = mx.full(
            (logits.shape[0], logits.shape[-1] - valid_vocab_size),
            float("-inf"),
            dtype=logits.dtype,
        )
        return mx.concatenate([logits[:, :valid_vocab_size], invalid], axis=-1)
    ...
    def generate(self, tokens, num_samples=1, max_tokens=None, temperature=1.0, top_k=None, seed=42, repetition_penalty=1.0):
        ...
        logits = self._mask_invalid_token_logits(logits)
        mx.eval(logits)
